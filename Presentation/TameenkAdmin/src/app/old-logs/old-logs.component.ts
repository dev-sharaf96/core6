...
import { RequestsNewFilter } from '../requests-new/requests-new-filter.model';
import { RequestNewModel } from '../requests-new/requests-new.model';
...
@Component({
  selector: 'app-old-logs',
  templateUrl: './old-logs.component.html',
  styleUrls: ['./old-logs.component.css']
})
export class OldLogsComponent implements OnInit {
  insuredtypeValue;
  moduleTypValue;
  moduleType;
  ...
  exportCsv() {
    if (!this.loading) {
      ...
      this._requestsService.getExcelWithFilterFromOldDB(this.requestsFilter).subscribe((data) => {
        if (data.data) {
  ...
  requestsLazyLoad(event) {
    ...
    this.loading = true;
    this._requestsService.getAllServicesWithFilterFromOldDB(this.requestsFilter,
