...
namespace Tameenk.Services.AdministrationApi.Controllers
{
    ...
    public class AutoleasingWalletController : AdminBaseApiController
    {
        ...
        public IHttpActionResult GetAllBanksWithFilter([FromBody] BankFilterModel userFilter, int pageIndex = 0, int pageSize = int.MaxValue)
        {
        ...
        public IHttpActionResult EditBank([FromBody] AutoleasingWalletUpdateModel model)
        {
        ...
        [HttpGet]
        public IHttpActionResult ToggleCompanyActivationByType(bool isActive, int bankId, int Walletstatues)
        {
            DateTime dtBeforeCalling = DateTime.Now;
            AdminRequestLog log = new AdminRequestLog();
            log.UserIP = Utilities.GetUserIPAddress();
