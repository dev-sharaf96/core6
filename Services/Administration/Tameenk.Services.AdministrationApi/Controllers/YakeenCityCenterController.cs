...
namespace Tameenk.Services.AdministrationApi.Controllers
{
    ...
    public class YakeenCityCenterController : AdminBaseApiController
    {
        ...
        public IHttpActionResult AddNew([FromBody]YakeenCityCenter model)
        {
            try
            {
        ...
        public IHttpActionResult GetVehicleMakers(int cityId, string cityName, int zipCode, int elmCode, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            try
            {
        ...
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(CommonResponseModel<IEnumerable<YakeenCityCenter>>))]
        public IHttpActionResult ExportAllNewServiceRequest(int cityId, string cityName, int zipCode, int elmCode)
        {
            try
            {
