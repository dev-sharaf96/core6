...
namespace Tameenk.Services.AdministrationApi.Controllers
{
    public class UserClaimsController : AdminBaseApiController
    {
        ...
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(IHttpActionResult))]
        ...
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(IHttpActionResult))]
        [AdminAuthorizeAttribute(pageNumber: 0)]
        public IHttpActionResult GetAllClaimsStatus(string lang)
        {
            string currentUserId = _authorizationService.GetUserId(User);
            if (string.IsNullOrEmpty(currentUserId))
                currentUserId = User.Identity.GetUserId();
        ...
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(IHttpActionResult))]
        [AdminAuthorizeAttribute(pageNumber: 0)]
        public IHttpActionResult GetUserClaimDetails(ClaimsFilter claimsFilter, string lang)
        {
            string currentUserId = _authorizationService.GetUserId(User);
            if (string.IsNullOrEmpty(currentUserId))
                currentUserId = User.Identity.GetUserId();
        ...
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(IHttpActionResult))]
        [AdminAuthorizeAttribute(pageNumber: 0)]
        public IHttpActionResult UpdateClaims(ClaimsUpdateModel model, string lang)
        {
            string currentUserId = _authorizationService.GetUserId(User);
            if (string.IsNullOrEmpty(currentUserId))
                currentUserId = User.Identity.GetUserId();
        ...
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(IHttpActionResult))]
        [AdminAuthorizeAttribute(pageNumber: 0)]
        public IHttpActionResult GetClaimRequesterTypes()
        {
            try
            {
