...
using NajmInsuranceInquiryService;
...
namespace Tameenk.Services.Inquiry.Components
{
    public class NajmService : INajmService
    {
        ...
        public async Task<NajmResponse> GetNajm(NajmRequest request, ServiceRequestLog predefinedLogInfo)
        {
            var najmResponse = await GetNajmResponse(request, predefinedLogInfo);
        ...
        protected async Task<NajmOutput> GetNajmResponse(NajmRequest request, ServiceRequestLog log)
        {
            ...
            try
            {
                ...
                string responseData = await serviceClient.NCDEligibilityAsync(request.PolicyHolderNin, request.IsVehicleRegistered ? 1 : 2, request.VehicleId);
        ...
        public async Task<NajmDriverCaseResponse> GetDriverCaseDetailV2(string driverId, int insuranceId)
        {
            var client = NajmNewServiceInit();
            var response = await client.GetDriverCaseDetailV2Async(driverId, insuranceId);
            var result = GetNajmResponseFromXml<NajmDriverCaseResponse>(response);
            return result;
        }

        public async Task<NajmVehicleCaseResponse> GetVehicleCaseDetailV2(string vehiclePlateNo, int? RegistrationType, int insuranceID)
        {
            var client = NajmNewServiceInit();
            var response = await client.GetVehicleCaseDetailV2Async(vehiclePlateNo, RegistrationType, insuranceID);
        ...
        public async Task<NajmOutput> GetDriverCaseDetailV2(NajmDriverCaseRequest request,string channel,string referenceId,string vehicleId,string externalId, Guid userId, string userName,int companyId,string companyName)
        {
            NajmOutput output = new NajmOutput();
            ...
            try
            {
                ...
                {
                    serviceClient.ClientCredentials.UserName.UserName = "NajmNewServiceUsername";// Utilities.GetAppSetting("NajmNewServiceUsername");
                    serviceClient.ClientCredentials.UserName.Password = "NajmNewServicePassword";// Utilities.GetAppSetting("NajmNewServicePassword");By Atheer
                   // log.ServiceURL = serviceClient.Endpoint.ListenUri.AbsoluteUri;
                    DateTime dtBeforeCalling = DateTime.Now;
                    string responseData = await serviceClient.GetDriverCaseDetailV2Async(request.DriverId, request.InsuranceId);
                    DateTime dtAfterCalling = DateTime.Now;
                    log.ServiceResponseTimeInSeconds = dtAfterCalling.Subtract(dtBeforeCalling).TotalSeconds;
                    log.ServiceResponse = responseData;
                    if (string.IsNullOrEmpty(responseData))
